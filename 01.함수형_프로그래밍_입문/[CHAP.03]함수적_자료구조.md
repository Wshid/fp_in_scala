...
## 3.5. 트리
- `List`는 소위 **대수적 자료 형식**(algebraic data type, ADT)의 한 예
  - 다른 분야에서의 추상 자료 형식(abstract data type)과는 다름
- **ADT**는 하나 이상의 **짜료 생성자**들로 이루어진 **자료 형식**을 의미
  - 자료 생성자들은 각각 `0`개 이상의 인수를 받을 수 있음
- 이러한 **자료 형식**을 해당 **자료 생성자**들의 `sum`또는 `union`이라 부름
  - 각각의 자료 생성자는 해당 인수들의 `product`라고 함

### 튜플
- `pair`나 `tuple` 역시 **대수적 자료 형식**
- `ADT`와 동일한 방식으로 작동
- 단 `p._1`, `p._3`과 같은 특수한 메서드가 존재 하며
  - `case class`와 비슷한 방식으로 **패턴 부합** 가능
- 대수적 자료 형식을, 다른 자료 구조 정의에 사용하기
- 예제 : 이진 트리 자료구조
  ```scala
  sealed trait Tree[+A]
...
### ADT와 캡슐화
- **대수적 자료 형식**이 형식의 내부 표현을 `public`으로 노출하므로
  - `encapsulation`을 위반한다는 불평이 있을 수 있음
- `fp`에서는 **캡슐화**문제를 다르게 취급함
  - 그 내부를 노출한다고 해도 **버그**로 이어지거나, **불변식**(invariant)이 위반될만한 `mutable`상태가 별로 없음
- 그러한 노출의 결정은 **자료 형식의 API**중 
  - 어떤 것을 공용으로 노출할 것인지 결정하는 것과 상당히 비슷한 방식
- `fp`에서는 `ADT`를 주로 경우 문들의 집합이 닫힌 상황에서 사용
  - `List`와 `Tree`의 경우
    - **자료 생성자**들의 집합을 변경하면,
      - 해당 **자료 형식**의 본성이 크게 달라짐
  - `List`는 단일 연결 목록
  - `Nil`과 `Cons`라는 두 경우는, `List`의 유용한 **공용 API** 일부를 형성
- `List`보다 더 추상적인 **API**를 다루는 코드를 작성하는 것도 물론 가능하지만,
  - 이런 종류의 **정보 은폐**는 `List`에 직접 박아 넣는 대신 **개별적인 계층**에서 처리 가능

## 3.6. 요약
- 이번 장에서는 중요한 개념이 여럿 등장
  - **대수적 자료 형식**과 **패턴 부합**을 소개
  - 단일 연결 목록을 비롯한 **순수 함수적 자료 구조 구현**