...
- 함수적 설계는 **반복적인 과정**
  - 하나의 `prototype`을 구현했다면,
  - 그것을 점점 복잡한 또는 현실적인 시나리오들에 사용해 봐야함
  - 그러다 보면 새로운 조합기를 필요로하는 **시나리오**를 만나게 됨
- 단, 필요한 조합기를 바로 구현하는 것 보다,
  - 가장 **일반적인 형태**로 정련할 수 있는지 살펴보는것이 바람직
- 그럴경우, 새로운 조합기가 아닌
  - 좀 더 일반적인 조합기의 `특별한 경우`가 필요했던 것일 뿐을 깨닫게 됨
- 예시: 두 분기 계산 중 하나를 **초기 계산의 결과**에 기초해서 선택하는 함수
  ```scala
  def choice[A](cond: Par[Boolean])(t: Par[A], f: Par[A]): Par[A]
  ```
  - `cond`의 결과가 `true`이면, `t`를 사용하여 계산 진행
  - `cond`의 결과가 `false`이면 `f`를 사용하여 계산 진행
- 위 내용을 `cond`의 결과가 나올 떄까지 실행을 **차단**하고
  - 그 결과를 이용해서 `t|f`의 실행을 결정하는 식으로도 구현 가능
- 간단한 차단식의 구현
  ```scala
  def choice[A](cond: Par[Boolean])(t: Par[A], f: Par[A]): Par[A] =
    es =>
      if (run(es)(cond).get) t(es) // cond의 결과가 나올때까지 차단
      else f(es)
  ```
- 조합기를 조금 더 생각해보기
  - 이 조합기는 `cond`를 실행하고, 결과가 나오면 `t|f` 실행
  - `Boolean`을 사용하는 것은 다소 **자의적**
  - 가능한 두 병렬 계산 `t`와 `f`중 하나를 선택하는 것도 **자의적**
  - 꼭 두 개일 필요가 있을까?
    - 어떤 **조건**에 기초하여 두 병렬 계산중 하나를 선택하는 것이 우용하다면
    - `N`개의 계산 중 하나를 선택하는 것도 유용함
- 조금 더 유용한 조합기
  ```scala
  def choiceN[A](n: Par[Int])(choices: List[Par[A]]): Par[A]
  ```
  - 이 함수는 `n`을 실행하고, 그 결과에 기초하여 `choices`의 병렬 계산 중 하나를 선택
    - `choice`보다 조금 더 일반적
- 조금 더 확장하기
- `List`와 마찬가지로, 가능한 선택의 집합을 `Map`으로 표현하더라도 필요 이상으로 **구체적**
- `choiceMap`의 구현을 살펴보면
  - `Map`의 `API`는 그리 많이 사용하지 않음
  - `Map[A, Par[B]]`는 단지 `A => Par[B]` 형식의 함수를 제공하는데 쓰임
- `choice`는 한 쌍의 인수들이 `Boolean => Par[A]`형식의 함수로 쓰이고,
  - `choiceN`에서는 `Int => Par[A]`형식의 함수로 쓰임
- 조금 더 통합한 일반적인 서명
  ```scala
  def chooser[A, B](pa: Par[A])(choices: A => Par[B]): Par[B]
  ```
- 함수를 위와 같이 **일반화**할 때는
  - 일반화의 결과로 나온 함수를 **비판적**으로 살펴볼 것
- 이런 함수가 `구체적인 용례`에서 비롯되었다고 하더라도,
  - 그 **서명**과 **구현**에는 더 **일반적**인 의미가 있을 수 있음
- `chooser`는 아마도, 이 연산의 가장 일반적인 이름은 아님
  - `chooser`는 일단 실행되면
    - 첫 계산을 실행하고
    - 그 결과에 따라 둘째 계산을 결정하는
    - 하나의 **병렬 계산**
  - 첫 계산의 결과가 준비되기 전에
    - 둘째 계산이 반드시 존재해야할 필요는 없음
- `List, Map`같은 컨테이너에 저장해 둘 필요가 없는 것
- 첫 계산의 결과를 이용하여 **둘째 계산**을 즉석에서 생성할 수도 있음
- 함수적 라이브러리에서 흔히 등장하는 그러한 함수를 흔히 `bind`나 `flatMap`이라 부름
  ```scala
  def flatMap[A,B](a: Par[A])(f: A => Par[B]): Par[B]
  ```
- 이 `flatMap`이 가장 기본적인 함수일까? 더 일반화가 가능할까?
- `flatMap`이라는 이름은, **두 단계**로 분해할 수 있음
  - 첫 단계: `Par[A]`에 `f: A => Par[B]`를 사상하여, `Par[Par[B]]`를 생성하는 것
  - 둘째 단계: `Par[Par[B]]`를 평평하게 만들어서 `Par[B]`를 만들기
- 임의의 `x`에 대해 `Par[Par[X]]`를 `Par[X]`로 변환하는 **더욱 간단한 조합기**를 추가하면 됨
  ```scala
  def join[A](a: Par[Par[A]]): Par[A]
  ```
- 이번에도 그냥 형식을 따라왔던 형태
- 주어진 서명을 가진 함수를 요구하는 예를 살펴보다가
  - 그렇나 함수를 실제 만들어냄
- 함수의 이름을 `join`이라고 한 것은
  - 개념적으로 이 함수는 일단 실행되면 **내부 계산**을 실행하고
  - 그 계산이 완료되길 기다리며(`Thread.join`과 유사)
  - 그 결과를 돌려주기 때문

#### 추가적으로 고려할만한 몇가지
- `map2`의 서명이 같은 함수를 `flatMap`과 `unit`을 사용해서 구현이 가능할까?
  - 그런 함수의 의미는 `map2`와 어떻게 다를까?
- `join`이 이 **병렬 계산 대수**의 다른 기본수단들과 연관시키는
  - **법칙**들은 어떤것이 있을지
- 이 대수로 표현할 수 없는 **병렬 계산**이 있을지
  - 대수에 새 **기본수단**을 추가하지 않고 표현할 수 없는 계산 찾아보기

#### 대수의 표현력과 한계 인식
- fp를 계속 연습하다 보면
  - **대수로부터 표현할 수 있는 함수**와 그 대수의 **한계**를 인식하는 능력이 생김
- 앞 예제에서 `choice`라는 함수를
  - `map`, `map2`, `unit`만으로 **순수하게**표현할 수 없다는 점을 명백히 알 수 없음
- 또한 `choice`가 단지 `flatMap`의 특수 경우라는 점도 명백하지 않음
- 시간이 지나면 이 부분을 조금 더 일찍 인식 가능
- 필요한 조합기를 표현할 수 있도록 **대수**를 수정하는 방법 역시 빨라지게 됨
- 이 능력은 `API 설계 작업`에 도움이 됨
- 실무의 관점에서 `API`를 최소한의 **기본 함수**로 줄이는 능력은 대단히 유용
- `parMap`을 기존 조합기들을 이용해서 구현할때 언급했듯이,
  - 기본적인 조합기들은 다소 까다로운 논리를 **캡슐화**하고 있는 경우가 많으며
  - 그런 것들을 재사용하면 **까다로운 논리를 되풀이하여 다룰 필요가 없음**

## 7.6. 요약
- **병렬** 및 **비동기 계산**을 **순수 함수적 방식**으로 정리하기 위한 라이브러리 설계를 완성
- `4,5,6`장에서는 **관심사의 분리**라는 주제
  - 좀 더 구체적으로 말하면 계산의 **서술**과
  - 계산을 실제로 수행하는 **해석기**(interpreter)를 분리한다는 주체가 두드러짐
- 이번장에서는 그러한 원리가
  - **병렬 계산**을 서술하는 `Par`형식과 **스레드**들을 띄워서
  - 계산을 실행하는 개별적인 해석기 `run`을 갖춘 라이브러리의 설계에서
    - 실제로 어떻게 작동하는지 볼 수 있음